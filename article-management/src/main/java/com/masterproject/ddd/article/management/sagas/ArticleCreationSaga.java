...
public class ArticleCreationSaga {
    ...
    public void on(ArticleCreated event) {
        ...
        SagaLifecycle.associateWith("articleID", event.Id);
    ...
    public void on(UserProfileStatusCheckVerified event) {
        ...
        commandGateway.send(PublisheArticle.builder().Id(event.getArticleID()).build());
    }

    @EndSaga
    @SagaEventHandler(associationProperty = "Id", keyName = "articleID")
    ...
    public void on(UserProfileStatusCheckNotVerified event) {
        ...
        commandGateway.send(RejectArticle.builder().Id(event.getArticleID()).reason(reason).build());
    }
