...
public class UserProfileProjection {
    ...
    public void on(UserProfileCreated event) {
        ...
        var articleEntity = new UserProfile();
        articleEntity.setId(event.Id);
        articleEntity.setName(userDetails.getName());
        articleEntity.setUsername(userDetails.getUserName());
        articleEntity.setEmail(userDetails.getEmail());
        articleEntity.setStatus(UserStatus.NEW);
    ...
    public boolean on(DoesUserProfileWithUsernameOrEmailExist query) {
        ...
        var optionalUsername = userWithSameUsernames.stream()
                .filter(user -> user.getStatus().equals(UserStatus.ACTIVATED)).findAny();
        var optionalEmail = userWithSameEmail.stream().filter(user -> user.getStatus().equals(UserStatus.ACTIVATED))
                .findAny();
