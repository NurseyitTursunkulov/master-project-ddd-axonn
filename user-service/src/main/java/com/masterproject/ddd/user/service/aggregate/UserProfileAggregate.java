    ...
    public void handle(CheckUserProfileStatus command) {
        ...
        if (this.status.equals(UserStatus.VERIFIED)) {
            AggregateLifecycle.apply(
                    UserProfileStatusCheckVerified.builder().Id(command.Id).referenceID(command.referenceID).build());
        } else {
            AggregateLifecycle.apply(
                    UserProfileStatusCheckNotVerified.builder().Id(command.Id).referenceID(command.referenceID)
                            .build());
        }
    }
